...
if hospital_scripts:
    ...
    for hospital in hospital_scripts:
        ...
        if 'country' in ids.keys() and 'country' in params.keys():
            if str(ids['country']).lower() == str(params['country']):
                if 'admissions' in ids.keys():
                    ...
                    if(adm_script_id != ''):
                        ...
                        admissions_case = admissions_case + \
                            " WHEN scriptid = '{0}' THEN '{1}' ".format(
                                adm_script_id, hospital)
                    else:
                ...
                if 'discharges' in ids.keys():
                    ...
                    if(disc_script_id != ''):
                        ...
                        dicharges_case = dicharges_case + \
                            " WHEN scriptid = '{0}' THEN '{1}' ".format(
                                disc_script_id, hospital)
                    else:
                      # ADD DUMMY SO THAT IN WORST CASE SCENARIO WE WILL HAVE AT LEAST 2 DUMMIES IN THE TUPLE,MAKING IT A VALID TUPLE
                ...
                if 'neolabs' in ids.keys():
                    ...
                    if (neolab_script_id != ''):
                        ...
                        neolabs_case = neolabs_case + \
                            " WHEN scriptid ='{0}' THEN '{1}' ".format(
                                neolab_script_id, hospital)
                    else:
                        # ADD DUMMY SO THAT IN WORST CASE SCENARIO WE WILL HAVE AT LEAST 2 DUMMIES IN THE TUPLE,MAKING IT A VALID TUPLE
            ...
            log.error(
                "Please specify country in both `database.ini` and `hospitals.ini` files")
            sys.exit()
