...
4. Run `pip install "kedro[pandas.CSVDataSet]==0.17.0"`
...
14. validation_email_interval_days: An `OPTIONAL` number value used to determine how many days should pass before validation summary emails are sent again. Validation issues are still saved to the database on every run. The default is `2`. The first run sends immediately if validation issues exist and email recipients are configured.
15. mail_host, mail_username, mail_password, mail_from_address: `OPTIONAL` mail server settings required only if validation summary emails should be sent.
...
## EXAMPLE OF FULL `database.ini` FILE:
    ...
    mail_username = validation@example.org
    mail_password = email_password
    mail_from_address = validation@example.org
...
Validation also writes category-specific logs:
1. validation_tech.log and validation_tech_summary.log
2. validation_implementation.log and validation_implementation_summary.log
3. validation_compliance.log and validation_compliance_summary.log

Validation issues are also saved in the database for trend reporting:
1. derived.validation_runs
2. derived.validation_issues

The summary logs are emailed to the category-specific receiver lists configured in `database.ini`, using `validation_email_interval_days` to control how often summary emails are sent.
...
> `Having challenges with wheel or pillow`:
