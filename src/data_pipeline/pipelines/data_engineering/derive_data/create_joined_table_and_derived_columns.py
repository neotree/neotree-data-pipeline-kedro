...
def join_table():
    ...
    try:
        ...
        jn_adm_dis = adm_df.merge(dis_df, how='left', on=[
                                  'uid', 'facility'], suffixes=('', '_discharge'))
        ...
        for index, row in jn_adm_dis.iterrows():
            ...
            if (is_date(str(row['DateTimeDischarge.value']))
                ...
                DateTimeDischarge = dt.strptime(str(str(row['DateTimeDischarge.value']))[
                                                :-14].strip(), date_format)
                DateTimeAdmission = dt.strptime(str(str(row['DateTimeAdmission.value']))[
                                                :-14].strip(), date_format)
            ...
            if 'DateTimeDeath.value' in row and (is_date(str(row['DateTimeDeath.value']))
                                                 and is_date(str(row['DateTimeAdmission.value']))):
                DateTimeDeath = dt.strptime(str(str(row['DateTimeDeath.value']))[
                                            :-14].strip(), date_format)
                DateTimeAdmission = dt.strptime(str(str(row['DateTimeAdmission.value']))[
                                                :-14].strip(), date_format)
