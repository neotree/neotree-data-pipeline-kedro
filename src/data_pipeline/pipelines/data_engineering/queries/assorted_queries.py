...
def read_new_smch_admissions_query():
    return f'''
            select 
                *
            from derived.admissions where TO_DATE(cast("DateTimeAdmission.value"::TEXT),'YYYY-MM-DD') >='2021-02-01' AND facility = 'SMCH';'''

def read_new_smch_discharges_query():
    return f'''
            select 
                *
            from derived.discharges where TO_DATE(cast("DateTimeDischarge.value"::TEXT),'YYYY-MM-DD') >='2021-02-01' 
                or TO_DATE(cast("DateTimeDeath.value" ::TEXT),'YYYY-MM-DD')>'2021-02-01'  AND facility = 'SMCH';'''
